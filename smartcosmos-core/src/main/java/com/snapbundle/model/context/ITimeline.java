 ...
 * optional description. Timeline entries may be flagged as a highlight (arbitrarily interpreted
 * by the developer). The intent of a highlight is to let some timeline entries stand out over
 * their peers.
 * <p/>
 * Timeline entries may also be flagged as invisible, preserving the entry but
 ...
 * design. Timeline entries that are flagged as inactive are never returned in a general
 ...
 */
public interface ITimeline extends IAccountDomainResource<ITimeline>, INamedObject<ITimeline>, IGeoLocation, IReferentialObject
{
    long getTimestamp();
